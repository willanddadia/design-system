...
const props = [
  ...
  },
  {
    ...
    type: '() => void',
    default: '—',
    description: 'Called when the backdrop or close button is clicked, or Escape is pressed.',
  },
  { name: 'title', type: 'string', default: '—', description: 'Title shown in the modal header.' },
  {
    name: 'variant',
    type: "'fullscreen' | 'popup' | 'side'",
    default: "'popup'",
    description: 'Layout variant (use the named exports instead for clarity).',
  },
  {
    name: 'children',
    type: 'ReactNode',
...
export function ModalPage() {
  ...
  return (
    <div>
      ...
      <Section
        title="Variants"
        description="Import named exports to use a specific variant without passing the variant prop."
      >
        <Example
      ...
      <Section title="When to use each variant">
        <Grid cols={1} mdCols={3} gap="md">
          {[
            {
              name: 'PopupModal',
              desc: 'Confirmations, alerts, quick actions. Center of the screen, max width 2xl.',
            },
            {
              name: 'FullscreenModal',
              desc: 'Immersive editing workflows or complex multi-step forms.',
            },
            {
              name: 'SideModal',
