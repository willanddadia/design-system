...
// ─── Example Block ───────────────────────────────────────────────────────────
export function Example({ title, code, children }: { title?: string; code?: string; children: ReactNode }) {
    return (
        <div className="border-2 border-border rounded-xl overflow-hidden mb-6">
            {/* Preview */}
            <div className="p-6 bg-background">
                {title && <p className="text-xs uppercase tracking-widest text-muted-foreground mb-4 font-medium">{title}</p>}
                {children}
            </div>
            {/* Code */}
            {code && (
                ...
                <tbody>
                    {props.map((p, i) => (
                        <tr key={p.name} className={i % 2 === 0 ? '' : 'bg-muted/10'}>
                            ...
                            <td className="px-4 py-3 font-mono text-muted-foreground text-xs">{p.default ?? '—'}</td>
